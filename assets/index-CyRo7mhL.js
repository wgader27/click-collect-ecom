...
</div>`;let F={products:[]},J={};J.handler_clickOnProduct=function(e){const t=e.target.closest("[data-buy]");if(t&&t.dataset.buy!==void 0){let s=parseInt(t.dataset.buy);const n=F.products.find(a=>a.id===s);if(n&&u.addItem({id:n.id,name:n.name,description:n.description||n.descriptionShort,image:n.image,price:n.price})){A.updateCartBadge(document);const r=t.innerHTML;t.innerHTML="✓",t.disabled=!0,setTimeout(()=>{t.innerHTML=r,t.disabled=!1},2e3)}}};J.init=async function(e){return e&&e.id?F.products=await L.fetchAllByCategory(e.id):F.products=await L.fetchAll(),w.init(F.products)};let w={};w.init=function(e){let t=w.createPageFragment(e);return w.attachEvents(t),t};w.createPageFragment=function(e){let t=i(Pe);console.log("Creating page fragment with data:",e);const s=t.querySelector("#nbrproduct");s.textContent=`${e.length} produit${e.length>1?"s":""}`;let n=le.dom(e);return t.querySelector('slot[name="products"]').replaceWith(n),t};w.attachEvents=function(e){return e.firstElementChild.addEventListener("click",J.handler_clickOnProduct),e};function ce(e){return console.log("ProductsPage",e),J.init(e)}const Ie=`<article class="mx-auto max-w-[75rem] bg-white text-text font-body">
...
</article>`;let de={html:function(e){return P(Ie,e)},dom:function(e){let t=i(de.html(e));const s=t.querySelector("#p-main"),n=t.querySelector("#p-thumbs"),a=t.querySelector("#p-dots");var r=["default.png"];e&&e.images&&Array.isArray(e.images)&&e.images.length?r=e.images:e&&e.image&&(r=[e.image]);const o="/click-collect-ecom/";if(s&&e&&e.id&&(s.src=`${o}assets/images/products/${e.id}/${r[0]}`,s.alt=e.name||""),n){n.innerHTML="";for(var l=0;l<r.length;l++){var f=r[l],Y=document.createElement("li"),x=document.createElement("button");x.type="button",x.style="border:none; background:none; padding:0; margin:0;",x.setAttribute("data-src",`${o}assets/images/products/${e.id}/${f}`),x.className=l===0?"bullets":"bullets-grey";var V=document.createElement("img");V.src=`${o}assets/images/products/${e.id}/${f}`,V.alt=e.name||"Image produit",V.style="width:70%; aspect-ratio:4/5; object-fit:cover; cursor: pointer;",x.appendChild(V),(function(v,_){_.addEventListener("click",function(){s&&(s.src=_.getAttribute("data-src"));for(var y=n.querySelectorAll("button"),g=0;g<y.length;g++)g===v?y[g].className="bullets":y[g].className="bullets-grey";if(a)for(var M=a.querySelectorAll("button"),b=0;b<M.length;b++)M[b].style.opacity=b===v?"1":".3"})})(l,x),Y.appendChild(x),n.appendChild(Y)}}if(a){a.innerHTML="";for(var D=0;D<r.length;D++){var $=document.createElement("button");$.type="button",$.className="h-2 w-2 rounded-full bg-black",$.style.opacity=D===0?"1":".3",(function(v){$.addEventListener("click",function(){const _="/click-collect-ecom/";if(s&&(s.src=`${_}assets/images/products/${e.id}/${r[v]}`),n)for(var y=n.querySelectorAll("button"),g=0;g<y.length;g++)g===v?y[g].className="bullets":y[g].className="bullets-grey";for(var M=a.querySelectorAll("button"),b=0;b<M.length;b++)M[b].style.opacity=b===v?"1":".3"})})(D),a.appendChild($)}}return t}};const Ae=`<div class="px-4 md:px-6 lg:px-8 py-4">
...
`;let q={cart:{}},E={};E.handleQuantityChange=function(e){const t=e.target,s=parseInt(t.dataset.qty),n=parseInt(t.value);u.updateQuantity(s,n),m.refreshCartContent(),A.updateCartBadge(document)};E.handleRemoveItem=function(e){const t=e.target.closest("button[data-remove]");if(!t)return;const s=parseInt(t.dataset.remove);confirm("Voulez-vous retirer cet article du panier ?")&&(u.removeItem(s),m.refreshCartContent(),A.updateCartBadge(document))};E.init=async function(){return q.cart=u.getState(),m.init()};let m={};m.init=function(){const e=i(Oe),t=e.querySelector('slot[name="cart-content"]');if(q.cart.isEmpty)t.replaceWith(i(Te));else{const s=m.createCartContent();t.replaceWith(s),m.attachEvents(e)}return e};m.createCartContent=function(){const e=q.cart.itemCount,t=e>1?"articles":"article",s={itemsCount:e,itemsCountLabel:t,total:q.cart.total.toFixed(2)};let n=P(Ve,s);const a=i(n),r=a.querySelector('slot[name="items"]'),o=m.createItemsList();return r.replaceWith(o),a};m.createItemsList=function(){const e=document.createDocumentFragment();return q.cart.items.forEach(t=>{const s=m.createItemDOM(t);e.appendChild(s)}),e};m.createItemDOM=function(e){let t=De;t=t.replace(/{{id}}/g,e.id||""),t=t.replace(/{{image}}/g,e.image||"default.png"),t=t.replace(/{{name}}/g,e.name||"Produit"),t=t.replace(/{{description}}/g,e.description||""),t=t.replace(/{{unitPrice}}/g,(e.price||0).toFixed(2));const s=((e.price||0)*(e.quantity||1)).toFixed(2);t=t.replace(/{{lineTotal}}/g,s);for(let n=1;n<=5;n++){const a=e.quantity===n?" selected":"";t=t.replace(`{{q${n}}}`,a)}return i(t).firstElementChild};m.attachEvents=function(e){e.querySelectorAll("select[data-qty]").forEach(n=>{n.addEventListener("change",E.handleQuantityChange)}),e.querySelectorAll("button[data-remove]").forEach(n=>{n.addEventListener("click",E.handleRemoveItem)})};m.refreshCartContent=function(){q.cart=u.getState();const e=document.querySelector("main");if(!e)return;for(;e.firstChild;)e.removeChild(e.firstChild);const t=m.init();e.appendChild(t)};function _e(){return E.init()}const Fe=`<section class="px-4 md:px-6 lg:px-8 py-6">
  <header class="mb-6">
...
`,Ne=`<article class="bg-white border border-black/20 rounded-lg p-8 shadow-sm md:mr-50 md:my-10">
...
`;let xe={html:function(e){const t=new Date(e.created_at),s=parseFloat(e.total).toFixed(2),n={"order-id":e.id,"order-number":e.id,"order-date":t.toLocaleDateString("fr-FR"),"order-total":s,"order-status":e.status};return P(He,n)},dom:function(e){const t=i(xe.html(e)),s=t.querySelector(".order-card");if(s){let n="";e.status==="En Cours"?n="background-color: var(--clr-purple); color: var(--clr-white);":e.status==="Validé"?n="background-color: var(--clr-green); color: var(--clr-white);":e.status==="Annulé"&&(n="background-color: var(--clr-red); color: var(--clr-white);");const a=s.querySelector(".order-status");a&&n&&a.setAttribute("style",n)}return t}},N={orders:[]},G={};G.init=async function(e){const t=await c.getCurrentUser();if(!t||!t.authenticated){window.location.href="/signin";return}return N.orders=await I("orders?user_id="+t.user.id),!N.orders||N.orders.length===0?B.initEmpty():B.init()};G.handleOrderClick=function(e){const t=e.currentTarget.dataset.orderId;localStorage.setItem("lastOrderId",t),window.location.href="/order-confirmation"};let B={};B.init=function(){const e=i(be),t=e.getElementById("orders-list");return N.orders.forEach(s=>{t.appendChild(xe.dom(s))}),t.querySelectorAll(".order-card").forEach(s=>{s.addEventListener("click",G.handleOrderClick)}),e};B.initEmpty=function(){const e=i(be);return e.getElementById("orders-list").classList.add("hidden"),e.getElementById("no-orders").classList.remove("hidden"),e};function We(e){return G.init(e)}const Je=`<div class="min-h-screen flex flex-col">
