...
</div>`;let B={products:[]},Z={};Z.handler_clickOnProduct=function(e){const t=e.target.closest("[data-buy]");if(t&&t.dataset.buy!==void 0){let n=parseInt(t.dataset.buy);const s=B.products.find(a=>a.id===n);if(s&&u.addItem({id:s.id,name:s.name,description:s.description||s.descriptionShort,image:s.image,price:s.price})){A.updateCartBadge(document);const r=t.innerHTML;t.innerHTML="✓",t.disabled=!0,setTimeout(()=>{t.innerHTML=r,t.disabled=!1},2e3)}}};Z.init=async function(e){return e&&e.id?B.products=await S.fetchAllByCategory(e.id):B.products=await S.fetchAll(),w.init(B.products)};let w={};w.init=function(e){let t=w.createPageFragment(e);return w.attachEvents(t),t};w.createPageFragment=function(e){let t=i(Pe);console.log("Creating page fragment with data:",e);const n=t.querySelector("#nbrproduct");n.textContent=`${e.length} produit${e.length>1?"s":""}`;let s=le.dom(e);return t.querySelector('slot[name="products"]').replaceWith(s),t};w.attachEvents=function(e){return e.firstElementChild.addEventListener("click",Z.handler_clickOnProduct),e};function ce(e){return console.log("ProductsPage",e),Z.init(e)}const Ie=`<article class="mx-auto max-w-[75rem] bg-white text-text font-body">
...
</article>`;let de={html:function(e){return P(Ie,e)},dom:function(e){let t=i(de.html(e));const n=t.querySelector("#p-main"),s=t.querySelector("#p-thumbs"),a=t.querySelector("#p-dots");var r=["default.png"];e&&e.images&&Array.isArray(e.images)&&e.images.length?r=e.images:e&&e.image&&(r=[e.image]);const o="/click-collect-ecom/";if(n&&e&&e.id&&(n.src=`${o}assets/images/products/${e.id}/${r[0]}`,n.alt=e.name||""),s){s.innerHTML="";for(var l=0;l<r.length;l++){var f=r[l],Y=document.createElement("li"),x=document.createElement("button");x.type="button",x.style="border:none; background:none; padding:0; margin:0;",x.setAttribute("data-src",`${o}assets/images/products/${e.id}/${f}`),x.className=l===0?"bullets":"bullets-grey";var D=document.createElement("img");D.src=`${o}assets/images/products/${e.id}/${f}`,D.alt=e.name||"Image produit",D.style="width:70%; aspect-ratio:4/5; object-fit:cover; cursor: pointer;",x.appendChild(D),(function(v,F){F.addEventListener("click",function(){n&&(n.src=F.getAttribute("data-src"));for(var y=s.querySelectorAll("button"),g=0;g<y.length;g++)g===v?y[g].className="bullets":y[g].className="bullets-grey";if(a)for(var M=a.querySelectorAll("button"),b=0;b<M.length;b++)M[b].style.opacity=b===v?"1":".3"})})(l,x),Y.appendChild(x),s.appendChild(Y)}}if(a){a.innerHTML="";for(var V=0;V<r.length;V++){var $=document.createElement("button");$.type="button",$.className="h-2 w-2 rounded-full bg-black",$.style.opacity=V===0?"1":".3",(function(v){$.addEventListener("click",function(){const F="/click-collect-ecom/";if(n&&(n.src=`${F}assets/images/products/${e.id}/${r[v]}`),s)for(var y=s.querySelectorAll("button"),g=0;g<y.length;g++)g===v?y[g].className="bullets":y[g].className="bullets-grey";for(var M=a.querySelectorAll("button"),b=0;b<M.length;b++)M[b].style.opacity=b===v?"1":".3"})})(V),a.appendChild($)}}return t}};const Ae=`<div class="px-4 md:px-6 lg:px-8 py-4">
...
`;let me={html:function(e){const t=e.reduce((s,a)=>s+a.price*a.quantity,0),n={"cart-count":e.length,"cart-total":t.toFixed(2)};return P(Ue,n)},dom:function(e){return i(me.html(e))}},T={cart:{}},G={};G.handleValidate=async function(){const e=await c.getCurrentUser();if(!e||!e.authenticated){window.location.href="/signin";return}const t={user_id:e.user.id,total:T.cart.total,items:T.cart.items.map(s=>({product_id:s.id,quantity:s.quantity,unit_price:s.price}))};console.log("Envoi commande:",t);const n=await ae("orders",JSON.stringify(t));console.log("Réponse complète:",n),console.log("order.id:",n?n.id:"order est null"),console.log("Type de order.id:",typeof(n?n.id:null)),n&&n.id?(localStorage.setItem("lastOrderId",n.id),u.clear(),window.location.href="/order-confirmation"):(console.error("Pas d'ID dans la réponse!",n),alert("Erreur lors de la validation"))};G.init=async function(){if(T.cart=u.getState(),T.cart.isEmpty){window.location.href="/cart";return}return pe.init()};let pe={};pe.init=function(){const e=i(Be);return e.getElementById("checkout-summary").appendChild(me.dom(T.cart.items)),e.getElementById("validate-order").addEventListener("click",G.handleValidate),e};function ze(){return G.init()}const fe=`<main class="px-4 md:px-6 lg:px-8 py-6">
...
`;let ge={html:function(e){const t=new Date(e.created_at),n=parseFloat(e.total).toFixed(2);let s="";e.items&&e.items.length>0&&(s=e.items.map(f=>`
            ...
            `).join(""));const a={"order-number":e.id,"order-date":t.toLocaleDateString("fr-FR"),"order-status":e.status,"order-total":n};let r=P(_e,a);const o=i(r),l=o.querySelector('section[id="order-items"]');return l&&s&&(l.innerHTML=s),o.innerHTML||r},dom:function(e){return i(ge.html(e))}},K={order:null},he={};he.init=async function(e){const t=localStorage.getItem("lastOrderId");return!t||(K.order=await I("orders/"+t),localStorage.removeItem("lastOrderId"),!K.order)?R.initError():R.init()};let R={};R.init=function(){const e=i(fe);return e.getElementById("order-detail").appendChild(ge.dom(K.order)),e};R.initError=function(){const e=i(fe),t=e.getElementById("order-detail");return t.innerHTML='<p class="text-error">Commande introuvable</p>',e};function Ne(e){return he.init(e)}const be=`<main class="px-4 md:px-6 lg:px-8 py-6">
  <header class="mb-6">
...
`;let xe={html:function(e){const t=new Date(e.created_at),n=parseFloat(e.total).toFixed(2),s={"order-id":e.id,"order-number":e.id,"order-date":t.toLocaleDateString("fr-FR"),"order-total":n,"order-status":e.status};return P(He,s)},dom:function(e){const t=i(xe.html(e)),n=t.querySelector(".order-card");if(n){let s="";e.status==="En Cours"?s="background-color: var(--clr-purple); color: var(--clr-white);":e.status==="Validé"?s="background-color: var(--clr-green); color: var(--clr-white);":e.status==="Annulé"&&(s="background-color: var(--clr-red); color: var(--clr-white);");const a=n.querySelector(".order-status");a&&s&&a.setAttribute("style",s)}return t}},U={orders:[]},J={};J.init=async function(e){const t=await c.getCurrentUser();if(!t||!t.authenticated){window.location.href="/signin";return}return U.orders=await I("orders?user_id="+t.user.id),!U.orders||U.orders.length===0?z.initEmpty():z.init()};J.handleOrderClick=function(e){const t=e.currentTarget.dataset.orderId;localStorage.setItem("lastOrderId",t),window.location.href="/order-confirmation"};let z={};z.init=function(){const e=i(be),t=e.getElementById("orders-list");return U.orders.forEach(n=>{t.appendChild(xe.dom(n))}),t.querySelectorAll(".order-card").forEach(n=>{n.addEventListener("click",J.handleOrderClick)}),e};z.initEmpty=function(){const e=i(be);return e.getElementById("orders-list").classList.add("hidden"),e.getElementById("no-orders").classList.remove("hidden"),e};function We(e){return J.init(e)}const Ze=`<div class="min-h-screen flex flex-col">
