...
`,_e=`<article class="bg-white border border-black/20 rounded-lg p-8 shadow-sm md:mr-50 md:my-10">
...
`,Ne=`<article class="border border-black/20 p-6 mb-8 text-left">
...
`;let xe={html:function(e){const t=new Date(e.created_at),n=parseFloat(e.total).toFixed(2),s={"order-id":e.id,"order-number":e.id,"order-date":t.toLocaleDateString("fr-FR"),"order-total":n,"order-status":e.status};return P(He,s)},dom:function(e){const t=i(xe.html(e)),n=t.querySelector(".order-card");if(n){let s="";e.status==="En Cours"?s="background-color: var(--clr-purple); color: var(--clr-white);":e.status==="Validé"?s="background-color: var(--clr-green); color: var(--clr-white);":e.status==="Annulé"&&(s="background-color: var(--clr-red); color: var(--clr-white);");const a=n.querySelector(".order-status");a&&s&&a.setAttribute("style",s)}return t}},_={orders:[]},G={};G.init=async function(e){const t=await c.getCurrentUser();if(!t||!t.authenticated){window.location.href="/signin";return}return _.orders=await I("orders?user_id="+t.user.id),!_.orders||_.orders.length===0?U.initEmpty():U.init()};G.handleOrderClick=function(e){const t=e.currentTarget.dataset.orderId;localStorage.setItem("lastOrderId",t),window.location.href="/order-confirmation"};let U={};U.init=function(){const e=i(be),t=e.getElementById("orders-list");return _.orders.forEach(n=>{t.appendChild(xe.dom(n))}),t.querySelectorAll(".order-card").forEach(n=>{n.addEventListener("click",G.handleOrderClick)}),e};U.initEmpty=function(){const e=i(be);return e.getElementById("orders-list").classList.add("hidden"),e.getElementById("no-orders").classList.remove("hidden"),e};function We(e){return G.init(e)}const Ze=`<div class="min-h-screen flex flex-col">
...
`;let Je={html:function(){return ne},dom:function(){return i(ne)}};function Qe(){let e=i(Ze),t=A.dom(),n=Je.dom();return e.querySelector('slot[name="header"]').replaceWith(t),e.querySelector('slot[name="footer"]').replaceWith(n),e}const Ge=` <section>
