...
</div>`;function we(){return ve}let P=function(e,t){let n=e;for(let s in t)n=n.replaceAll("{{"+s+"}}",t[s]);return n};function i(e){const t=document.createElement("template");return t.innerHTML=e.trim(),t.content}const ke=`<main class="font-body text-text">
  <!-- Hero Section -->
...
`;let D="https://www.w-gader.mmi-limoges.fr/api-ecom/",I=async function(e){let t={method:"GET",credentials:"include"};try{var n=await fetch(D+e,t)}catch(a){return console.error("Echec de la requête : "+a),!1}return n.status!=200?(console.error("Erreur de requête : "+n.status),!1):await n.json()},ae=async function(e,t){let n={credentials:"include",method:"POST",header:{"Content-Type":"multipart/form-data"},body:t};try{var s=await fetch(D+e,n)}catch(r){return console.error("Echec de la requête : "+r),!1}let a=await s.json();return s.status!=200&&console.error("Erreur de requête : "+s.status,a),a},Ce=async function(e,t){let n={method:"POST",header:{"Content-Type":"application/json"},body:t};try{var s=await fetch(D+e,n)}catch(r){return console.error("Echec de la requête : "+r),!1}let a=await s.json();return s.status!=200&&console.error("Erreur de requête : "+s.status,a),a},re=async function(e){let t={method:"DELETE",credentials:"include"};try{var n=await fetch(D+e,t)}catch(a){return console.error("Echec de la requête : "+a),!1}return n.status!=200?(console.error("Erreur de requête : "+n.status),!1):await n.json()},oe=async function(e,t){let n={method:"PATCH",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)};try{var s=await fetch(D+e,n)}catch(r){return console.error("Echec de la requête : "+r),!1}let a=await s.json();return s.status!=200&&console.error("Erreur de requête : "+s.status,a),a},c={};c.login=async function(e){try{const t=await ae("auth",JSON.stringify(e));if(t)return t;throw new Error("No response")}catch{return console.warn("API Login failed (likely CORS), using MOCK fallback."),sessionStorage.setItem("mock_auth","true"),{success:!0,user:{id:1,email:"mock@user.com",firstname:"Mock",lastname:"User"}}}};c.signup=async function(e){try{const t=await Ce("users",JSON.stringify(e));if(t)return t;throw new Error("No response")}catch{return console.warn("API Signup failed, using MOCK fallback."),sessionStorage.setItem("mock_auth","true"),{id:1,email:e.email}}};c.getCurrentUser=async function(){try{const e=await I("auth");if(e)return e;throw new Error("No auth")}catch{return sessionStorage.getItem("mock_auth")==="true"?{authenticated:!0,user:{id:1,firstname:"Mock",lastname:"User",email:"mock@user.com"}}:{authenticated:!1}}};c.logout=async function(){return sessionStorage.removeItem("mock_auth"),await re("auth")};c.updateProfile=async function(e){const t=await oe("auth",e);return console.log("Update profile response:",t),t};c.changePassword=async function(e,t){const n=await oe("auth/password",{current_password:e,new_password:t});return console.log("Change password response:",n),n};c.deleteAccount=async function(){const e=await c.getCurrentUser();if(!e||!e.authenticated||!e.user)return{success:!1,error:"Utilisateur non connecté"};const t=e.user.id,n=await re(`users/${t}`);return console.log("Delete account response:",n),n&&await c.logout(),n};const ie={};ie.init=async function(){const e=i(ke);try{const t=await c.getCurrentUser();if(t&&t.authenticated&&t.user){const s=t.user.firstname||"",a=e.querySelector("h1");a&&s&&(a.textContent=`Ravie de vous revoir ${s}`)}}catch(t){console.error("Erreur lors de la récupération de l'utilisateur:",t)}return e};async function Le(){return await ie.init()}let S={},Y=[{id:1,name:"Marteau",description:"Un marteau est un outil utilisé pour enfoncer des clous dans du bois ou d'autres matériaux. Il se compose d'une tête lourde en métal fixée à un manche en bois ou en fibre de verre.",price:9.99},{id:2,name:"Tournevis",description:"Un tournevis est un outil utilisé pour visser ou dévisser des vis. Il se compose d'une tige en métal avec une tête qui s'adapte à la fente de la vis.",price:5.99},{id:3,name:"Clé à molette",description:"Une clé à molette est un outil utilisé pour serrer ou desserrer des écrous et des boulons. Elle se compose d'une mâchoire réglable qui s'adapte à différentes tailles d'écrous.",price:12.99},{id:4,name:"Pince",description:"Une pince est un outil utilisé pour saisir, tenir ou plier des objets. Elle se compose de deux bras articulés qui se rejoignent en un point de pivot.",price:7.99},{id:5,name:"Scie",description:"Une scie est un outil utilisé pour couper des matériaux, généralement en bois. Elle se compose d'une lame dentée fixée à un manche.",price:14.99},{id:6,name:"Perceuse",description:"Une perceuse est un outil utilisé pour percer des trous dans divers matériaux. Elle se compose d'un moteur qui fait tourner une mèche.",price:49.99},{id:7,name:"Ponceuse",description:"Une ponceuse est un outil utilisé pour lisser des surfaces en bois ou en métal. Elle se compose d'un moteur qui fait vibrer ou tourner un abrasif.",price:79.99},{id:8,name:"Mètre",description:"Un mètre est un outil utilisé pour mesurer des distances. Il se compose d'une bande graduée en métal ou en plastique.",price:19.99},{id:9,name:"Niveau à bulle",description:"Un niveau à bulle est un outil utilisé pour vérifier l'horizontalité ou la verticalité d'une surface. Il se compose d'un tube rempli de liquide avec une bulle d'air à l'intérieur.",price:9.99}];S.fetch=async function(e){let t=await I("products/"+e);return t==!1?Y.pop():[t]};S.fetchAll=async function(){let e=await I("products");return e==!1?Y:e};S.fetchAllByCategory=async function(e){let t=await I(`products?category=${e}`);return t==!1?Y:t};let u={},p=[];function Se(){let e=localStorage.getItem("cart");e&&(p=JSON.parse(e))}function Z(){localStorage.setItem("cart",JSON.stringify(p))}function qe(){let e=0;for(let t of p)e+=t.price*t.quantity;return{total:e}}u.addItem=function(e){const t=p.find(n=>n.id===e.id);return t?t.quantity<5&&t.quantity++:p.push({id:e.id,name:e.name||"Produit",description:e.description||"",image:e.image||"default.png",price:e.price||0,quantity:1}),Z(),!0};u.updateQuantity=function(e,t){let n=p.find(s=>s.id===e);n&&(n.quantity=t,Z())};u.removeItem=function(e){p=p.filter(t=>t.id!==e),Z()};u.clear=function(){p=[],Z()};u.getItems=function(){return p};u.getItemCount=function(){let e=0;for(let t of p)e+=t.quantity;return e};u.isEmpty=function(){return p.length===0};u.getState=function(){let e=qe();return{items:p,itemCount:u.getItemCount(),total:e.total,isEmpty:p.length===0}};Se();const ee=`<header class="bg-white text-black border-b border-black/10 font-body">
...
</header>`;let A={html:function(){return ee},dom:function(){let e=i(ee);const t=e.querySelector("#navBtn"),n=e.querySelector("#drawer"),s=e.querySelector("#backdrop"),a=e.querySelector("#iconOpen"),r=e.querySelector("#iconClose");t&&n&&s&&(t.onclick=()=>{n.classList.toggle("hidden"),a.classList.toggle("hidden"),r.classList.toggle("hidden")},s.onclick=()=>{n.classList.add("hidden"),a.classList.remove("hidden"),r.classList.add("hidden")});const o=window.location.pathname;e.querySelectorAll("a[data-link]").forEach($=>$.classList.remove("active")),o==="/products"?e.querySelector('a[href="/products"]').classList.add("active"):o.startsWith("/category/1")?e.querySelector('a[href="/category/1"]').classList.add("active"):o.startsWith("/category/2")?e.querySelector('a[href="/category/2"]').classList.add("active"):o.startsWith("/category/3")&&e.querySelector('a[href="/category/3"]').classList.add("active");const f=e.querySelector('a[href="/my-account/dashboard"]');return f&&(window.router&&window.router.isAuthenticated||f.setAttribute("href","/signin")),A.updateCartBadge(e),e},updateCartBadge:function(e=document){const t=e.querySelector("#cart-badge");if(!t)return;const n=u.getItemCount();t.textContent=n,n>0?(t.classList.remove("hidden"),t.classList.add("flex")):(t.classList.add("hidden"),t.classList.remove("flex"))}};const Ee=`<article class="relative">
...
</article>`;let le={html:function(e){let t='<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">';for(let n of e)n.image=n.images&&n.images.length>0?n.images[0]:"default.png",n.descriptionShort=n.desription&&n.description.length>30?n.description.substring(0,30)+"...":n.description,t+=P(Ee,n);return t+"</div>"},dom:function(e){return i(le.html(e))}};const Pe=`<div class="mx-auto max-w-7xl px-4 md:px-6 lg:px-8 py-4">
...
</div>`;let U={products:[]},Q={};Q.handler_clickOnProduct=function(e){const t=e.target.closest("[data-buy]");if(t&&t.dataset.buy!==void 0){let n=parseInt(t.dataset.buy);const s=U.products.find(a=>a.id===n);if(s&&u.addItem({id:s.id,name:s.name,description:s.description||s.descriptionShort,image:s.image,price:s.price})){A.updateCartBadge(document);const r=t.innerHTML;t.innerHTML="✓",t.disabled=!0,setTimeout(()=>{t.innerHTML=r,t.disabled=!1},2e3)}}};Q.init=async function(e){return e&&e.id?U.products=await S.fetchAllByCategory(e.id):U.products=await S.fetchAll(),w.init(U.products)};let w={};w.init=function(e){let t=w.createPageFragment(e);return w.attachEvents(t),t};w.createPageFragment=function(e){let t=i(Pe);console.log("Creating page fragment with data:",e);const n=t.querySelector("#nbrproduct");n.textContent=`${e.length} produit${e.length>1?"s":""}`;let s=le.dom(e);return t.querySelector('slot[name="products"]').replaceWith(s),t};w.attachEvents=function(e){return e.firstElementChild.addEventListener("click",Q.handler_clickOnProduct),e};function ce(e){return console.log("ProductsPage",e),Q.init(e)}const Ie=`<article class="mx-auto max-w-[75rem] bg-white text-text font-body">
...
</article>`;let de={html:function(e){return P(Ie,e)},dom:function(e){let t=i(de.html(e));const n=t.querySelector("#p-main"),s=t.querySelector("#p-thumbs"),a=t.querySelector("#p-dots");var r=["default.png"];e&&e.images&&Array.isArray(e.images)&&e.images.length?r=e.images:e&&e.image&&(r=[e.image]);const o="/click-collect-ecom/";if(n&&e&&e.id&&(n.src=`${o}assets/images/products/${e.id}/${r[0]}`,n.alt=e.name||""),s){s.innerHTML="";for(var l=0;l<r.length;l++){var f=r[l],$=document.createElement("li"),x=document.createElement("button");x.type="button",x.style="border:none; background:none; padding:0; margin:0;",x.setAttribute("data-src",`${o}assets/images/products/${e.id}/${f}`),x.className=l===0?"bullets":"bullets-grey";var V=document.createElement("img");V.src=`${o}assets/images/products/${e.id}/${f}`,V.alt=e.name||"Image produit",V.style="width:70%; aspect-ratio:4/5; object-fit:cover; cursor: pointer;",x.appendChild(V),(function(v,B){B.addEventListener("click",function(){n&&(n.src=B.getAttribute("data-src"));for(var y=s.querySelectorAll("button"),g=0;g<y.length;g++)g===v?y[g].className="bullets":y[g].className="bullets-grey";if(a)for(var R=a.querySelectorAll("button"),b=0;b<R.length;b++)R[b].style.opacity=b===v?"1":".3"})})(l,x),$.appendChild(x),s.appendChild($)}}if(a){a.innerHTML="";for(var F=0;F<r.length;F++){var M=document.createElement("button");M.type="button",M.className="h-2 w-2 rounded-full bg-black",M.style.opacity=F===0?"1":".3",(function(v){M.addEventListener("click",function(){const B="/click-collect-ecom/";if(n&&(n.src=`${B}assets/images/products/${e.id}/${r[v]}`),s)for(var y=s.querySelectorAll("button"),g=0;g<y.length;g++)g===v?y[g].className="bullets":y[g].className="bullets-grey";for(var R=a.querySelectorAll("button"),b=0;b<R.length;b++)R[b].style.opacity=b===v?"1":".3"})})(F),a.appendChild(M)}}return t}};const Ae=`<div class="px-4 md:px-6 lg:px-8 py-4">
 ...
 `;let T={products:[]};T.getProductById=function(e){return T.products.find(t=>t.id==e)};let G={};G.handler_clickOnProduct=function(e){if(e.target.dataset.buy!==void 0){let t=parseInt(e.target.dataset.buy);const n=T.getProductById(t);if(n&&u.addItem({id:n.id,name:n.name,description:n.description||n.descriptionShort,image:n.image,price:n.price})){A.updateCartBadge(document);const a=e.target,r=a.textContent;a.textContent="✓ Ajouté au panier",a.disabled=!0,setTimeout(()=>{a.textContent=r,a.disabled=!1},2e3)}}};G.init=async function(e){const t=e.id;T.products=await S.fetchAll();let n=T.getProductById(t);return console.log("Product loaded:",n),n&&(n.image=n.images&&Array.isArray(n.images)&&n.images.length?n.images[0]:n.image||"default.png"),k.init(n)};let k={};k.init=function(e){let t=k.createPageFragment(e);return k.attachEvents(t),t};k.createPageFragment=function(e){let t=i(Ae),n=de.dom(e);return t.querySelector('slot[name="detail"]').replaceWith(n),t};k.attachEvents=function(e){return e.querySelector("[data-buy]").addEventListener("click",G.handler_clickOnProduct),e};function $e(e){return console.log("ProductDetailPage",e),G.init(e)}const Me=`<main class="px-4 md:px-6 lg:px-8 py-6">
...
`;let Re={html:function(){return te},dom:function(){return i(te)}},ue={};ue.render=function(e){let t=Me.replace("{{name}}",e.firstname);const n=i(t),s=n.querySelector('slot[name="dashboard-cards"]');s&&s.replaceWith(Re.dom());const a=n.querySelector("#logout-btn");return a&&a.addEventListener("click",async()=>{await c.logout(),window.location.href="/signin"}),n};async function je(){const e=await c.getCurrentUser();if(!e||!e.authenticated){window.location.href="/signin";return}return ue.render(e.user)}const Te=`<main class="px-4 md:px-6 lg:px-8 py-6">
  <a href="/products" data-link class="inline-flex items-center gap-2 text-sm mb-6">
...
`,Oe=`<section class="font-body text-text py-12 text-center">
  <figure class="mb-6">
...
`;let q={cart:{}},E={};E.handleQuantityChange=function(e){const t=e.target,n=parseInt(t.dataset.qty),s=parseInt(t.value);u.updateQuantity(n,s),m.refreshCartContent(),A.updateCartBadge(document)};E.handleRemoveItem=function(e){const t=e.target.closest("button[data-remove]");if(!t)return;const n=parseInt(t.dataset.remove);confirm("Voulez-vous retirer cet article du panier ?")&&(u.removeItem(n),m.refreshCartContent(),A.updateCartBadge(document))};E.init=async function(){return q.cart=u.getState(),m.init()};let m={};m.init=function(){const e=i(Te),t=e.querySelector('slot[name="cart-content"]');if(q.cart.isEmpty)t.replaceWith(i(Oe));else{const n=m.createCartContent();t.replaceWith(n),m.attachEvents(e)}return e};m.createCartContent=function(){const e=q.cart.itemCount,t=e>1?"articles":"article",n={itemsCount:e,itemsCountLabel:t,total:q.cart.total.toFixed(2)};let s=P(De,n);const a=i(s),r=a.querySelector('slot[name="items"]'),o=m.createItemsList();return r.replaceWith(o),a};m.createItemsList=function(){const e=document.createDocumentFragment();return q.cart.items.forEach(t=>{const n=m.createItemDOM(t);e.appendChild(n)}),e};m.createItemDOM=function(e){let t=Ve;t=t.replace(/{{id}}/g,e.id||""),t=t.replace(/{{image}}/g,e.image||"default.png"),t=t.replace(/{{name}}/g,e.name||"Produit"),t=t.replace(/{{description}}/g,e.description||""),t=t.replace(/{{unitPrice}}/g,(e.price||0).toFixed(2));const n=((e.price||0)*(e.quantity||1)).toFixed(2);t=t.replace(/{{lineTotal}}/g,n);for(let s=1;s<=5;s++){const a=e.quantity===s?" selected":"";t=t.replace(`{{q${s}}}`,a)}return i(t).firstElementChild};m.attachEvents=function(e){e.querySelectorAll("select[data-qty]").forEach(s=>{s.addEventListener("change",E.handleQuantityChange)}),e.querySelectorAll("button[data-remove]").forEach(s=>{s.addEventListener("click",E.handleRemoveItem)})};m.refreshCartContent=function(){q.cart=u.getState();const e=document.querySelector("main");if(!e)return;for(;e.firstChild;)e.removeChild(e.firstChild);const t=m.init();e.appendChild(t)};function Fe(){return E.init()}const Be=`<section class="px-4 md:px-6 lg:px-8 py-6">
  <header class="mb-6">
...
`,Ue=`<article class="bg-white border border-black/20 rounded-lg p-8 shadow-sm md:mr-50 md:my-10">
...
`;let me={html:function(e){const t=e.reduce((s,a)=>s+a.price*a.quantity,0),n={"cart-count":e.length,"cart-total":t.toFixed(2)};return P(Ue,n)},dom:function(e){return i(me.html(e))}},O={cart:{}},J={};J.handleValidate=async function(){const e=await c.getCurrentUser();if(!e||!e.authenticated){window.location.href="/signin";return}const t={user_id:e.user.id,total:O.cart.total,items:O.cart.items.map(s=>({product_id:s.id,quantity:s.quantity,unit_price:s.price}))};console.log("Envoi commande:",t);const n=await ae("orders",JSON.stringify(t));console.log("Réponse complète:",n),console.log("order.id:",n?n.id:"order est null"),console.log("Type de order.id:",typeof(n?n.id:null)),n&&n.id?(localStorage.setItem("lastOrderId",n.id),u.clear(),window.location.href="/order-confirmation"):(console.error("Pas d'ID dans la réponse!",n),alert("Erreur lors de la validation"))};J.init=async function(){if(O.cart=u.getState(),O.cart.isEmpty){window.location.href="/cart";return}return pe.init()};let pe={};pe.init=function(){const e=i(Be);return e.getElementById("checkout-summary").appendChild(me.dom(O.cart.items)),e.getElementById("validate-order").addEventListener("click",J.handleValidate),e};function ze(){return J.init()}const fe=`<main class="px-4 md:px-6 lg:px-8 py-6">
...
`,_e=`<article class="border border-black/20 p-6 mb-8 text-left">
...
`;let ge={html:function(e){const t=new Date(e.created_at),n=parseFloat(e.total).toFixed(2);let s="";e.items&&e.items.length>0&&(s=e.items.map(f=>`
            ...
            `).join(""));const a={"order-number":e.id,"order-date":t.toLocaleDateString("fr-FR"),"order-status":e.status,"order-total":n};let r=P(_e,a);const o=i(r),l=o.querySelector('section[id="order-items"]');return l&&s&&(l.innerHTML=s),o.innerHTML||r},dom:function(e){return i(ge.html(e))}},X={order:null},he={};he.init=async function(e){const t=localStorage.getItem("lastOrderId");return!t||(X.order=await I("orders/"+t),localStorage.removeItem("lastOrderId"),!X.order)?j.initError():j.init()};let j={};j.init=function(){const e=i(fe);return e.getElementById("order-detail").appendChild(ge.dom(X.order)),e};j.initError=function(){const e=i(fe),t=e.getElementById("order-detail");return t.innerHTML='<p class="text-error">Commande introuvable</p>',e};function Ne(e){return he.init(e)}const be=`<main class="px-4 md:px-6 lg:px-8 py-6">
  <header class="mb-6">
...
`;let xe={html:function(e){const t=new Date(e.created_at),n=parseFloat(e.total).toFixed(2),s={"order-id":e.id,"order-number":e.id,"order-date":t.toLocaleDateString("fr-FR"),"order-total":n,"order-status":e.status};return P(He,s)},dom:function(e){const t=i(xe.html(e)),n=t.querySelector(".order-card");if(n){let s="";e.status==="En Cours"?s="background-color: var(--clr-purple); color: var(--clr-white);":e.status==="Validé"?s="background-color: var(--clr-green); color: var(--clr-white);":e.status==="Annulé"&&(s="background-color: var(--clr-red); color: var(--clr-white);");const a=n.querySelector(".order-status");a&&s&&a.setAttribute("style",s)}return t}},z={orders:[]},K={};K.init=async function(e){const t=await c.getCurrentUser();if(!t||!t.authenticated){window.location.href="/signin";return}return z.orders=await I("orders?user_id="+t.user.id),!z.orders||z.orders.length===0?_.initEmpty():_.init()};K.handleOrderClick=function(e){const t=e.currentTarget.dataset.orderId;localStorage.setItem("lastOrderId",t),window.location.href="/order-confirmation"};let _={};_.init=function(){const e=i(be),t=e.getElementById("orders-list");return z.orders.forEach(n=>{t.appendChild(xe.dom(n))}),t.querySelectorAll(".order-card").forEach(n=>{n.addEventListener("click",K.handleOrderClick)}),e};_.initEmpty=function(){const e=i(be);return e.getElementById("orders-list").classList.add("hidden"),e.getElementById("no-orders").classList.remove("hidden"),e};function We(e){return K.init(e)}const Ze=`<div class="min-h-screen flex flex-col">
    ...
    `,ne=`<footer class="border-t border-text/10 bg-bg font-body text-text">
  <!-- Footer principal -->
...
`;let Qe={html:function(){return ne},dom:function(){return i(ne)}};function Ge(){let e=i(Ze),t=A.dom(),n=Qe.dom();return e.querySelector('slot[name="header"]').replaceWith(t),e.querySelector('slot[name="footer"]').replaceWith(n),e}const Je=` <section>
    <h1>404 - Page non trouvée</h1>
        <p>Cette page n'existe pas</p>
    <nav>
        <a href="/click-collect-ecom/" data-link>Retour à l'accueil</a>
    </nav>
</section>`;function Ke(){return Je}const Xe=`<main class="mx-auto w-full max-w-[22.5rem] px-3 pt-3 pb-10 md:max-w-[26.25rem] md:px-4 font-body text-text">
...
</main>`,st={dom(){return i(nt)}},at=`<main class="px-4 py-4 md:px-6 lg:px-8">
...
`;let H={};H.handleFormSubmit=async function(e){e.preventDefault(),e.stopPropagation();const t=e.target,n=t.querySelector("#error-message");if(!n)return;n.classList.add("hidden"),n.textContent="";let s=new FormData(t);const a={email:s.get("email"),password:s.get("password")},r=await c.login(a);r&&r.success?(window.router&&window.router.setAuth(!0),window.router.navigate("/")):r&&r.error?(n.textContent=r.error,n.classList.remove("hidden")):(n.textContent="Erreur de connexion",n.classList.remove("hidden"))};H.togglePassword=function(e){const t=e.currentTarget,n=t.closest(".relative").querySelector("input"),s=t.querySelector("#eye-closed"),a=t.querySelector("#eye-open");n.type==="password"?(n.type="text",s.classList.add("hidden"),a.classList.remove("hidden")):(n.type="password",s.classList.remove("hidden"),a.classList.add("hidden"))};let W={};W.attachEvents=function(e){const t=e.querySelector("form"),n=e.querySelector("#toggle-password");t&&t.addEventListener("submit",H.handleFormSubmit),n&&n.addEventListener("click",H.togglePassword)};W.init=function(){const e=i(at),t=st.dom();return e.querySelector('slot[name="form"]').replaceWith(t),W.attachEvents(e),e};function rt(){return W.init()}const ot=`<main class="px-4 py-4 md:px-6 lg:px-8">
...
</section>`,lt={dom(){return i(it)}},h={};h.handleInfoFormSubmit=async function(e){e.preventDefault(),e.stopPropagation();const t=e.target,n=new FormData(t),s={civ:n.get("civ"),lastname:n.get("lastname"),firstname:n.get("firstname"),email:n.get("email")};let a;try{a=await c.updateProfile(s)}catch(r){console.error("Erreur:",r),alert("Erreur réseau");return}if(a&&a.success)alert("Profil mis à jour avec succès !");else{const r=a&&a.error?a.error:"Impossible de mettre à jour le profil";alert(r)}};h.handlePasswordFormSubmit=async function(e){e.preventDefault(),e.stopPropagation();const t=e.target,n=new FormData(t),s=n.get("current_password"),a=n.get("new_password");let r;try{r=await c.changePassword(s,a)}catch(o){console.error("Erreur:",o),alert("Erreur réseau");return}if(r&&r.success)alert("Mot de passe changé avec succès !"),t.reset(),t.querySelectorAll('[id^="rule-"]').forEach(l=>{l.classList.remove("password-rule-valid"),l.classList.add("password-rule-invalid")});else{const o=r&&r.error?r.error:"Impossible de changer le mot de passe";alert(o)}};h.handleDeleteAccount=async function(e){if(e.preventDefault(),!confirm("Êtes-vous sûr de vouloir supprimer définitivement votre compte ?"))return;let n;try{n=await c.deleteAccount()}catch(s){console.error("Erreur:",s),alert("Erreur réseau lors de la suppression du compte");return}n?(alert("Votre compte a été supprimé avec succès"),window.location.href="/"):alert("Erreur lors de la suppression du compte")};h.togglePassword=function(e){const t=e.currentTarget,n=t.closest(".relative"),s=n?n.querySelector("input[type='password'], input[type='text']"):null,a=t.querySelector(".eye-icon:not(.hidden)"),r=t.querySelector(".eye-icon.hidden");s&&(s.type==="password"?(s.type="text",a&&a.classList.add("hidden"),r&&r.classList.remove("hidden")):(s.type="password",r&&r.classList.add("hidden"),a&&a.classList.remove("hidden")))};h.handlePasswordInput=function(e){const t=e.target.value,n=e.target.closest("form"),s=n.querySelector("#rule-length"),a=n.querySelector("#rule-uppercase"),r=n.querySelector("#rule-lowercase"),o=n.querySelector("#rule-number"),l=n.querySelector("#rule-special");t.length>=12?(s.classList.add("password-rule-valid"),s.classList.remove("password-rule-invalid")):(s.classList.remove("password-rule-valid"),s.classList.add("password-rule-invalid")),/[A-Z]/.test(t)?(a.classList.add("password-rule-valid"),a.classList.remove("password-rule-invalid")):(a.classList.remove("password-rule-valid"),a.classList.add("password-rule-invalid")),/[a-z]/.test(t)?(r.classList.add("password-rule-valid"),r.classList.remove("password-rule-invalid")):(r.classList.remove("password-rule-valid"),r.classList.add("password-rule-invalid")),/[0-9]/.test(t)?(o.classList.add("password-rule-valid"),o.classList.remove("password-rule-invalid")):(o.classList.remove("password-rule-valid"),o.classList.add("password-rule-invalid")),/[!@#$%^&*(),.?":{}|<>]/.test(t)?(l.classList.add("password-rule-valid"),l.classList.remove("password-rule-invalid")):(l.classList.remove("password-rule-valid"),l.classList.add("password-rule-invalid"))};const L={};L.loadUserData=async function(e){try{const t=await c.getCurrentUser();if(!t||!t.authenticated||!t.user){console.error("Utilisateur non connecté"),window.location.href="/signin";return}const n=t.user,s=e.querySelector(`input[name="civ"][value="${n.civ}"]`);s&&(s.checked=!0);const a=e.querySelector("#lastname");a&&(a.value=n.lastname||"");const r=e.querySelector("#firstname");r&&(r.value=n.firstname||"");const o=e.querySelector("#email");o&&(o.value=n.email||"")}catch(t){console.error("Erreur lors du chargement des données:",t)}};L.attachEvents=function(e){const t=e.querySelector("#form-profile-info");t&&t.addEventListener("submit",h.handleInfoFormSubmit);const n=e.querySelector("#form-profile-password");n&&n.addEventListener("submit",h.handlePasswordFormSubmit);const s=e.querySelector("#toggle-current-password");s&&s.addEventListener("click",h.togglePassword);const a=e.querySelector("#toggle-new-password");a&&a.addEventListener("click",h.togglePassword);const r=e.querySelector("#new-password");r&&r.addEventListener("input",h.handlePasswordInput);const o=e.querySelector("#delete-account");o&&o.addEventListener("click",h.handleDeleteAccount)};L.init=async function(){const e=i(ot),t=e.querySelector('slot[name="form"]');return t&&t.replaceWith(lt.dom()),await L.loadUserData(e),L.attachEvents(e),e};async function ct(){return await L.init()}const d=new ye("app",{loginPath:"/signin",basePath:"/click-collect-ecom/"});window.router=d;const se=await c.getCurrentUser();se&&se.authenticated?d.setAuth(!0):d.setAuth(!1);d.addLayout("/",Ge);d.addRoute("/",Le);d.addRoute("/about",we);d.addRoute("/products",ce);d.addRoute("/category/:id",ce);d.addRoute("/products/:id/:slug",$e);d.addRoute("/cart",Fe);d.addRoute("/checkout",ze,{requireAuth:!0});d.addRoute("/order-confirmation",Ne,{requireAuth:!0});d.addRoute("/signin",rt,{useLayout:!1});d.addRoute("/signup",tt,{useLayout:!1});d.addRoute("/my-account/dashboard",je,{requireAuth:!0});d.addRoute("/my-account/orders",We,{requireAuth:!0});d.addRoute("/my-account/profile",ct,{requireAuth:!0});d.addRoute("*",Ke);d.start();
